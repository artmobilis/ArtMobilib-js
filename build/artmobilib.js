...
(function() {
    ...
    function Parse(json) {
        return ParseResources(json).then(function(res) {
            return ParseObject(json.object, res.materials, res.geometries, res.constants.model_path).then(function(object) {
    ...
    function ParseArray(json) {
        return ParseResources(json).then(function(res) {
            return ParseObjectArray(json.objects, res.materials, res.geometries, res.constants.model_path);
        });
    }
    ...
    function GetGeometry(name, geometries) {
        if (geometries[name] !== undefined) return geometries[name]; else {
            if (name === undefined) console.warn("failed to get geometry: no id provided"); else console.warn("failed to get geometry: no such geometry: " + name);
        }
        return undefined;
    }
    function GetMaterial(name, materials) {
        if (materials[name] !== undefined) return materials[name]; else {
            if (name === undefined) console.warn("failed to get material: no id provided"); else console.warn("failed to get material: no such material: " + name);
        }
        return undefined;
    }
    ...
    function ParseObjectArray(json, materials, geometries, model_path) {
        ...
        } else return Promise.reject("failed to parse object array: json not an array");
    }
